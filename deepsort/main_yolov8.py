...
#Use this command: python main_yolov8.py {video path} {.pt yolo model path}
video_path = sys.argv[1]
yolo_model_path = sys.argv[2]
video_out_path = os.path.join('.', video_path.split("/")[-1].split(".")[0]+yolo_model_path.split("/")[-1].split(".")[0]+'_yolov8_out.mp4')
...
while ret:
    ...
    for result in results:#iterate each image   
        ...
        detections = []
        # class_id_list = []
        for r in result.boxes.data.tolist():#in one image, iterate detections
